...
export default function Projects() {
  ...
  // 根据搜索查询过滤项目
  const filteredProjects = searchQuery ? projectsService.searchProjects(searchQuery) : allProjects;

  return (
    <>
      <SEO
        title="Projects"
        description="Explore Sam Yang's portfolio of projects, featuring web applications, mobile apps, and other software solutions built with modern technologies."
      />
      <Layout>
          ...
          <Box sx={{ my: 4 }}>
            ...
            {filteredProjects.length > 0 ? (
                      ...
                      <CardContent sx={{ flexGrow: 1 }}>
                        ...
                        <Box sx={{ mb: 2 }}>
                          {project.technologies.map((tech) => (
                            <Chip key={tech} label={tech} size="small" sx={{ mb: 0.5, mr: 0.5 }} />
                          ))}
                        </Box>
              ...
              <Box sx={{ py: 6, textAlign: 'center' }}>
                ...
                <Button onClick={() => setSearchQuery('')} sx={{ mt: 2 }} variant="text">
                  Clear Search
                </Button>
              </Box>
            )}
